...
import { AccountModel, AddAccount, AddAccountModel, EmailValidator, Validator } from './signup-protocols'
...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 when passwordConfirmation is not provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return 400 when passwordConfirmation does not match the password', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
